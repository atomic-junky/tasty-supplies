...
execute if data entity @s SelectedItem{id:"minecraft:wooden_sword"} if data entity @s SelectedItem.components.minecraft:custom_model_data positioned ~ ~0.078125 ~-0.125 as @e[type=item_display, distance=..1, sort=nearest] at @s run return run function tasty_supplies:cutting_board/cut_item
...
# Else drop the item
execute unless data entity @s SelectedItem{id:"minecraft:wooden_sword"} run function tasty_supplies:cutting_board/drop_item/main
